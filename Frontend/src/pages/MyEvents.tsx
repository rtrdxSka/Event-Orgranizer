...
import { Loader2, XCircle, Calendar, Users, Plus, Clock, MapPin } from 'lucide-react';
...
const MyEvents = () => {
  ...
  // Get the next upcoming date for an event
  const getNextEventDate = (event: EventGet) => {
    if (!event.eventDates?.dates || event.eventDates.dates.length === 0) {
      return null;
    }

    // Filter to future dates and sort
    const futureDates = event.eventDates.dates
  ...
  const EventCard = ({ event, isCreator }: { event: EventGet, isCreator: boolean }) => {
      ...
      >
        <CardHeader className="pb-2">
          <div className="flex justify-between items-start">
            <CardTitle className="text-xl text-purple-100">{event.name}</CardTitle>
        ...
        <CardContent className="flex-1">
          <p className="text-purple-200 line-clamp-2 mb-4">{event.description}</p>
          
{/* Display event date */}
<div className="flex items-center gap-2 text-sm mb-2">
  <Clock className="h-4 w-4" />
  {event.eventDate ? (
    <span className="text-green-300">Date: {formatDate(event.eventDate)}</span>
  ) : (
        ...
        <CardFooter className="pt-2 border-t border-purple-700/30">
          <div className="flex items-center gap-1 text-sm text-purple-300">
            ...
            {otherResponsesCount > 0 ? (
              <span>{otherResponsesCount} {otherResponsesCount === 1 ? 'other response' : 'other responses'}</span>
            ) : (
    ...
    <div className="min-h-screen bg-purple-950">
      ...
      <div className="pt-20 pb-12 px-4">
        <div className="max-w-7xl mx-auto">
          ...
          <div className="mb-10">
            <h2 className="text-2xl font-semibold text-purple-100 mb-4">Events You Created</h2>
            {createdEvents.length > 0 ? (
              ...
              <div className="bg-purple-900/30 rounded-lg p-8 text-center">
                <p className="text-purple-200 mb-4">You haven't created any events yet.</p>
                <Button 
                  onClick={() => navigate('/event/create')}
                  className="bg-purple-200 text-purple-950 hover:bg-purple-100"
                >
          ...
          <div>
            <h2 className="text-2xl font-semibold text-purple-100 mb-4">Events You Responded To</h2>
            {respondedEvents.length > 0 ? (
              ...
              <div className="bg-purple-900/30 rounded-lg p-8 text-center">
                <p className="text-purple-200">You haven't responded to any events yet.</p>
              </div>
            )}
          </div>
        </div>
