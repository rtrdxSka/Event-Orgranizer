...
import { Mail, Calendar, AlertTriangle, Edit, Shield, User as UserIcon, CheckCircle } from 'lucide-react';
  ...
  <Alert className="mb-8 relative overflow-hidden bg-gradient-to-br from-purple-900/80 via-indigo-800/60 to-violet-900/80 border border-orange-500/40 backdrop-blur-xl shadow-xl">
    ...
    <div className="relative z-10 flex items-start gap-3">
      ...
      <div className="flex-1">
        <AlertTitle className="text-white font-semibold text-lg mb-2">Email Verification Required</AlertTitle>
        ...
        <Card className="mb-8 relative overflow-hidden bg-gradient-to-br from-purple-900/80 via-indigo-800/60 via-purple-800/70 to-violet-900/80 border border-purple-600/40 backdrop-blur-2xl shadow-2xl">
          ...
          <div className="absolute bottom-1/3 left-1/3 w-24 h-24 bg-indigo-400/5 rounded-full blur-2xl animate-pulse" style={{animationDelay: '1.5s'}}></div>
...
<div className="relative z-10">
  ...
  <div className="flex items-start gap-4 h-full">
    ...
    <div className="flex-1 flex flex-col justify-between">
      <div>
        <p className="text-purple-300 text-sm font-medium mb-1">Email Address</p>
      ...
      <div className="flex items-center gap-2 mt-2">
        {verified ? (
          <>
            ...
            <span className="text-emerald-300 text-sm font-medium">Verified Account</span>
          </>
        ) : (
          <>
            <div className="w-2 h-2 bg-amber-400 rounded-full animate-pulse" />
            <span className="text-amber-300 text-sm font-medium">Pending Verification</span>
          </>
        )}
      </div>
    </div>
  ...
  <div className="flex items-start gap-4 h-full">
    ...
    <div className="flex-1 flex flex-col justify-between">
      <div>
        ...
        <p className="text-white font-semibold text-lg">{formatDate(createdAt)}</p>
      </div>
      <div className="flex items-center gap-2 mt-2">
        <div className="w-2 h-2 bg-indigo-400 rounded-full" />
        <span className="text-indigo-300 text-sm font-medium">
          {Math.floor((new Date().getTime() - new Date(createdAt).getTime()) / (1000 * 60 * 60 * 24))} days active
        </span>
      </div>
    </div>
  </div>
    ...
    <div className="h-full flex flex-col">
      <div className="mb-8">
        ...
        <p className="text-purple-200 text-base leading-relaxed">
          Keep your account secure by updating your email address or changing your password regularly. Regular updates help protect against unauthorized access.
        </p>
      </div>
